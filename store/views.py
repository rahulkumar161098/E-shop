...
def home(request):
    ...
    # getting limitted data of a category from database
    limited_data= Product.objects.filter(category=4).order_by('data')[
        :6]
    print('limited data is ..............', limited_data)
    ...
    if request.method == 'POST':
        ...
        if u_name and pwd and f_pwd != '':
            ...
            if UserSignUp.objects.filter(u_name=u_name):
                messages.info(
                    request, 'User already exist Please try another!')
                ...
                if pwd == f_pwd:
                    ...
                    if len(pwd) >= 3:
                        ...
                        messages.info(
                            request, "Password should be 8 charaters!")
                        # print("length error")
...
def check_out(request):
    ...
    if request.method == 'POST':
        ...
        set_address = Address(user_id=user, name=name, mobile=mobile, local_address=address,
                              city=city, zip_code=pin, lend_mark=lend_mark, state=state)
        set_address.save()
...
# search by category, title and desc
def search(request):
    search_key = request.GET.get('search_area')
    product_list= Product.objects.all()
    if search_key is not '':
        search_by_category = Product.objects.filter(des__icontains=search_key)

        search_by_title = Product.objects.filter(name__icontains=search_key)

        all_search = search_by_category.union(search_by_title)
        ...
        if all_search.count() == 0:
            return HttpResponse('No product found', search_list)
        else:
            print('all product are : ' , all_search)
            return HttpResponse('All product are fetches', search_list)
    else:
        # messages.info(request,'Please write in search box ')
        return HttpResponse('all same product')


# getting limited data
# def limited_product(request):
#     l_data= Product.objects.filter(category='Mobiles')
#     print(l_data)
...
client = razorpay.Client(
    auth=("rzp_test_3Xjh9rRVSDJ3Lu", "HzIgyX5NwXq2BnUkx4ot3uLx"))
